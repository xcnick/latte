...
void InnerProductLayer<Dtype>::Backward_gpu(
  ...
  if (this->param_propagate_down_[0]) {
    ...
    if (transpose_) {
      latte_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, K_, N_, M_, (Dtype)1.,
                            bottom_data, top_diff, (Dtype)1.,
                            this->blobs_[0]->mutable_gpu_diff());
    } else {
      latte_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, N_, K_, M_, (Dtype)1.,
  ...
  if (bias_term_ && this->param_propagate_down_[1]) {
    ...
    latte_gpu_gemv<Dtype>(CblasTrans, M_, N_, (Dtype)1., top_diff,
  ...
  if (propagate_down[0]) {
    ...
    if (transpose_) {
      latte_gpu_gemm<Dtype>(CblasNoTrans, CblasTrans, M_, K_, N_, (Dtype)1.,
                            top_diff, this->blobs_[0]->gpu_data(), (Dtype)0.,
                            bottom[0]->mutable_gpu_diff());
    } else {
      latte_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, K_, N_, (Dtype)1.,
                            top_diff, this->blobs_[0]->gpu_data(), (Dtype)0.,
                            bottom[0]->mutable_gpu_diff());
    }
  }
